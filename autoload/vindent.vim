...
function! <SID>NoHang(range)
	...
	while <SID>Skip(1,l:range[0]) | let l:range[0]=l:range[0]+1 | endwhile
	while <SID>Skip(1,l:range[1]) | let l:range[1]=l:range[1]-1 | endwhile
	return l:range
endfunction
...
function! vindent#Object(code, skip, stop_func, count)
	...
	for l:time in range(a:count-1) " recursive
		...
		if l:zs!=l:range[0] && l:ze!=l:range[1] && <SID>Diff(<SID>Get(l:zs),l:ze)
			if <SID>More(<SID>Get(l:zs),l:ze) | let l:zs = l:range[0]
